...
import { RetrieveArtists, RetrieveEvents, RetrievePhotos, RetrieveNotifications, RetrieveLocation } from "../globalFuns";
import { USERS_COMPANY, CURRENT_USER } from "../constants";
import api from "../api";
...
function EventCreate(){
    ...
    const [voivodCorrect, setVoivodCorrect] = useState(false);
    ...
    const [townCorrect, setTownCorrect] = useState(false)
    ...
    const [streetCorrect, setStreetCorrect] = useState(false)
    ...
    const [eventTicketsLink, setEventTicketsLink] = useState("");
    ...
    const [cheapTicketPrice, setCheapTicketPrice] = useState("");
    const [expensiveTicketPrice, setExpensiveTicketPrice] = useState("");
    const [cheapConcessionTicketPrice, setCheapConcessionTicketPrice] = useState("");
    const [expensiveConcessionTicketPrice, setExpensiveConcessionTicketPrice] = useState("");
    ...
    const [primaryTagGroup, setPrimaryTagGroup] = useState([]);
    ...
    const [secondaryTagGroup, setSecondaryTagGroup] = useState([]);
    ...
    const handleNewArtistErrorMessage = () => {
        if(!newArtistName || !newArtistPhoto){
            ...
            const artist = { name: newArtistName, photo: newArtistPhoto, id:0, photoFile: newArtistPhotoFile};
    ...
    const deleteFromArtists = (target) => {
        setArtistGroup(prev =>
            prev.filter(item => item.name !== target)
        );
    };
    const artistCardButtonFunction = (name, photo, id) => {
        const artist = {name, photo, id, photoFile: null};
        setArtistGroup(prev => [...prev, artist]);
        setNewArtistName("");
        setNewArtistPhoto(null);
        setNewArtistErrorMessage("");
    ...
    const [canEveryoneEdit, setCanEveryoneEdit] = useState(false);
    ...
    const handleSubmition = async (e) => {
        ...
        if(
            ...
            try{
                ...
                const eventResponse = await api.post('/api/create_event/', payload);
                if(eventResponse.status==200 || eventResponse.status==201){
                    ...
                    for (const artist of artistGroup) {
                        ...
                        if(artist.id === 0){
                            ...
                            artistId = artist.id
                            const artistNotificationResponse = await api.post('/api/create_notification/', {
                                notificationDescription: `Dodano wydarzenie z udziałem ${artist.name}`, 
                                artist: artist.id,
                                event: eventId
                            });
                            if(artistNotificationResponse.status!==200 && artistNotificationResponse.status!==201){
                                setErrorMessage(artistNotificationResponse.data?.error || "Nieznany błąd");
                                return;
                            }
                        }
                        const addArtistResponse = await api.post(`/api/add_event_artist/${eventId}/`, {
                    ...
                    const allImages = [eventPhoto,...galleryImages]
                    for(const photo of allImages){
                        const photoFormData = new FormData()
                        photoFormData.append("image", photo)
                        photoFormData.append("eventImage", eventId)
                        const addPhotoResponse = await api.post(`/api/upload_photo/`, photoFormData, {
                            headers: {
                                'Content-Type': 'multipart/form-data'
                            }
                        });
                        if(addPhotoResponse .status!==200 && addPhotoResponse !==201){
                            setErrorMessage(addPhotoResponse .data?.error || "Nieznany błąd")
                            return                                
                        }
                    }
                    await RetrievePhotos()
                    const companyNotificationResponse = await api.post('/api/create_notification/', {notificationDescription: `Organizacja ${company.name} dodała nowe wydarzenie`, company: company.id, event: eventId})
                    ...
                    const newCompanyTags = company.tags ? company.tags  + tags : tags
    ...
    return(
        ...
        <main>
            <Form onSubmit={handleSubmition}>
                ...
                <div className="event-create-frame" style={{justifyContent: 'center', alignItems: 'center'}}>
                    ...
                    <div style={{width: '100%', display: !showTickets ? 'flex' : 'none', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
                        <InputGroup className="event-create-input-group">
                            ...
                            <Form.Control
                            className="event-create-input-control"
                            aria-label="Link do szczegółów wydarzenia"
                        ...
                        <InputGroup className="event-create-input-group">
                            ...
                            <Form.Control
                            className="event-create-input-control"ąąąą
                        ...
                        <div style={{width: '100%', display: showConcession ? 'flex' : 'none', flexDirection: 'column'}}>
                            ...
                            <InputGroup className="event-create-input-group">
                                ...
                                <Form.Control
                                className="event-create-input-control"
                                aria-label="Najdroższe bilety uglowe"
                    ...
                    {artistGroup.map((artist, index) =>(
                        <Card className="artist-card" key={index}>
                            <div className="artist-card-a">
                                <Card.Img className="artist-card-image" variant="top" src={artist.photo} alt="zdjęcie artysty"/>
                            </div>
                ...
                <div className="event-create-frame">
                    <InputGroup style={{height: '40vh'}}>
                        <Form.Control className="event-create-input-control" as="textarea" aria-label="Dodatkowe informacje dotyczące wydarzenia" value={eventAdditional} onChange={handleEventAdditional}/>
                    </InputGroup>
                </div>
                <div className="event-page-headlines">
                    <h2>
                ...
                <div className="event-create-frame" style={{justifyContent: 'center', alignItems: 'center'}}>
                    ...
                    <img src={eventPhotoPreview} className="event-create-promotional-photo" style={{border: '2px solid #4C2C72', borderRadius: '10px'}} alt="Wybrane zdjęcie wydarzenia"/>
                    <div className="text-muted login-form-control-error text-center" style={{display: eventPhoto ? 'none' : 'block'}}>
                        To pole musi być wypełnione
                    </div>
                </div>
                <div className="event-page-headlines">
